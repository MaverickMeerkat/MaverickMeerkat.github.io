...
The **Softmax** function is used in many machine learning applications for multi-class classifications. Unlike the Logistic Regression which is a binary classification algorithm that uses the **Sigmoid** function, which takes one input and assigns to it a number (the probability) from 0 to 1 that it's a YES, the softmax function can take many inputs and assign probability for each one. 
...
def sigmoid_back(da, z):
	# da being the derivative up to this point, and z the cached input of the sigmoid 
    a = sigmoid(z)
    return da * a * (1 - a)
```
