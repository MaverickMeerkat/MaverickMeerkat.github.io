...
Meaning, you update the value function Q for the current state S and action A, by taking a step (alpha) in the direction of the distance between the (bootstrapped) "real" (or better) value, and the old value. If alpha (the step size) is 1, you are basically replacing the old value with the new value. If it's less you are using a "moving" average. The new value is just the current reward + the value function for the new state and action. 
